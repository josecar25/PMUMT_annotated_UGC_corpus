...
for t in range(1,14):
    ...
    l_src_norm = [0] * 13300
    system = 'TX'
    out_file_src_norm = "./types/" + system + "/" + t + ".norm.fr"
    out_file_src_noisy = "./types/" + system + "/" + t + ".noisy.fr"

    out_file_ref_norm = "./types/" + system + "/" + t + ".norm.en"
    out_file_pred_norm = "./types/" + system + "/" + t + ".pred.norm.en"

    out_file_ref_noisy = "./types/" + system + "/" + t + ".noisy.en"
    out_file_pred_noisy = "./types/" + system + "/" + t + ".pred.noisy.en"

    count_norm = [0]*13300
    count_norm_tmp = 0

    for cc, l in zip(errors, range(0,13300)):
        cc_s = cc.strip('')

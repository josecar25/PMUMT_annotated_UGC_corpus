...
f_ref = open(args.out_dir+'ref.fr', 'w')
...
#        print(perm_corr)
    ...
    res_total = []
    for r in range(len(corrections_tmp)+1, len(corrections_tmp)+1):
        for i in itertools.combinations(corrections_tmp, r):
            res_total.append(list(i))
    if not res_total:
        res_total=[corrections_tmp]
    ...
    for line, elem in enumerate(res,1):
      ...
      #  ref_tmp = []
        ...
        for error in elem:
            ...
            ref_tmp = []
            errors_tmp = []
            set_corr = set(corrections_tmp)
            list_err = ((elem))
            set_err = set(list_err)
            err = list(set_corr-set_err)
            #print(error)
            if  error[0] == '9' or error[0] =='13' or error[0] == '6': #False: #error[0] == '10' or error[0] == '6' or error[0] == '9' or error[0] =='N':
        ...
        #        errors_tmp.append((error[0], error[1]))
         ...
         #           errors_tmp.append((error[0], error[1], error[2]))
            ...
            sents_tmp.append(' '.join(sent_t).replace('  ', ' ').replace(' ķ', ''))
                    #errors_out.append(error[0])
          #  errors_tmp.append((error[0], error[1]))
        sents_out.append(sents_tmp)
        errors_sent.append(errors_tmp)
        ref_out.append(ref_tmp)
        #errors_sent.append(errors_tmp)
    out_src.append(sents_out)
    print("generated_sent:\n")
    for elem, err, ref in zip(sents_out, errors_sent, ref_out): #, err in zip(sent_out, errors_sent):
        print(elem)
        print(err)
        print(ref)
        print("============")

...
#    out_src_norm = open(out_file_src_norm, "r")
    ...
    for i in range(0,tirages+1):
        ...
        f_p_norm = open('p_norm.tmp', 'w')
        f_r_norm = open('r_norm.tmp', 'w')
        f_p_noisy = open('p_noisy.tmp', 'w')
        f_r_noisy = open('r_noisy.tmp', 'w')
        ...
        r_norm = [r_norm]
        r_noisy = [r_noisy]
        stream_norm = os.popen('perl multi-bleu-detok.perl -lc r_norm.tmp < p_norm.tmp')
        stream_noisy = os.popen('perl multi-bleu-detok.perl -lc r_noisy.tmp < p_noisy.tmp')
    ...
    print("CI RATIO for system " + system + " for type " + t + " = " + str(ci_ratio) + " total: " +  str(round(((ci_ratio[1]+ci_ratio[0])/2.0),5)) + "+/-" + str(round(ci_ratio[1]-((ci_ratio[1]+ci_ratio[0])/2.0),5)))
    print("CI SCORE NORM for system " + system + " for type " + t + " = " + str(ci_scores_norm) + " total: " +  str(round(((ci_scores_norm[1]+ci_scores_norm[0])/2.0),5)) + "+/-" + str(round(ci_scores_norm[1]-((ci_scores_norm[1]+ci_scores_norm[0])/2.0),5)))
    print("CI SCORE NOISY for system " + system + " for type " + t + " = " + str(ci_scores_noisy) + " total: " +  str(round(((ci_scores_noisy[1]+ci_scores_noisy[0])/2.0),5)) + "+/-" + str(round(ci_scores_noisy[1]-((ci_scores_noisy[1]+ci_scores_noisy[0])/2.0),5)))
    print("-----------------")
    print("SacreBleu --tokenize=intl:")
    print("CI RATIO for system " + system + " for type " + t + " = " + str(ci_ratio_SB) + " total: " +  str(round(((ci_ratio_SB[1]+ci_ratio_SB[0])/2.0),5)) + "+/-" + str(round(ci_ratio_SB[1]-((ci_ratio_SB[1]+ci_ratio_SB[0])/2.0),5)))
    print("CI SCORE NORM for system " + system + " for type " + t + " = " + str(ci_scores_norm_SB) + " total: " +  str(round(((ci_scores_norm_SB[1]+ci_scores_norm_SB[0])/2.0),5)) + "+/-" + str(round(ci_scores_norm_SB[1]-((ci_scores_norm_SB[1]+ci_scores_norm_SB[0])/2.0),5)))
    print("CI SCORE NOISY for system " + system + " for type " + t + " = " + str(ci_scores_noisy_SB) + " total: " +  str(round(((ci_scores_noisy_SB[1]+ci_scores_noisy_SB[0])/2.0),5)) + "+/-" + str(round(ci_scores_noisy_SB[1]-((ci_scores_noisy_SB[1]+ci_scores_noisy_SB[0])/2.0),5)))
    print("#################")
    print("#################")
    print("#################")
    scores_norm = []
